...
SERVER_WORLD="world" # Server world directory
SERVER_WORLD_THE_END="world_the_end" # Server world directory
SERVER_WORLD_NETHER="world_nether" # Server world directory
SERVER_DIRECTORY="/server" # Server directory, NOT the world; world is SERVER_DIRECTORY/world
BACKUP_DIRECTORY="/backup" # Directory to save backups in
MAX_BACKUPS=-1 # -1 indicates unlimited
...
# Minecraft server rcon interface functions
message-players () {
  local MESSAGE=$1
...
execute-command () {
  local COMMAND=$1
  rcon-cli --config /home/.rcon-cli.yaml $COMMAND
}
...
case $COMPRESSION_ALGORITHM in
  "") # No compression
    tar -cf $ARCHIVE_PATH -C $SERVER_DIRECTORY ./$SERVER_WORLD ./$SERVER_WORLD_NETHER ./$SERVER_WORLD_THE_END
    ;;
  *) # With compression
    tar -cf - -C $SERVER_DIRECTORY ./$SERVER_WORLD ./$SERVER_WORLD_NETHER ./$SERVER_WORLD_THE_END | $COMPRESSION_ALGORITHM -cv -$COMPRESSION_LEVEL - > $ARCHIVE_PATH 2>> /dev/null
    ;;
esac
sync
END_TIME=$(date +"%s")
...
WORLD_SIZE_BYTES=$(du -b --max-depth=0 $SERVER_DIRECTORY/$SERVER_WORLD | awk '{print $1}')
